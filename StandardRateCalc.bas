...
Sub RunProgram1()
    ...
    Dim Rate As Double
    Rate = CDbl(InputBox("Enter rate per " & RateType & " (" & CurrSymbol & "):", "Rate Input", "0.00"))

    Dim MinFee As Double
    MinFee = CDbl(InputBox("Enter minimum session fee (" & CurrSymbol & "):" & vbCrLf & _
...
Private Function GetCurrency() As String
    ...
    msg = "Select your currency:" & vbCrLf & vbCrLf & _
          ...
          "8:  INR - Indian Rupee (ChrW(8377))" & vbCrLf & _
          "9:  BRL - Brazilian Real (R$)" & vbCrLf & _
          "10: KRW - South Korean Won (ChrW(8361))" & vbCrLf & _
    ...
    Select Case ans
        ...
        Case "6":  GetCurrency = "¥"
        Case "7":  GetCurrency = "¥"
        Case "8":  GetCurrency = ChrW(8377)
        Case "9":  GetCurrency = "R$"
        Case "10": GetCurrency = ChrW(8361)
...
Private Function GetCurrencyFormat(symbol As String) As String
    ...
    If symbol = "¥" Or symbol = ChrW(8361) Then
...
Sub BuildCharacterSummary(hs As Worksheet, charName As String, RateType As String, Rate As Double, MinFee As Double, Threshold As Long, CurrSymbol As String)
    ...
    ws.Cells(3, 4).Formula = "=ROUND(C3/1.875,1) / 86400"
...
Sub BuildFullCastSummary(hs As Worksheet, CharNames() As String, RateType As String, Rate As Double, MinFee As Double, Threshold As Long, CurrSymbol As String)
    ...
    For c = 0 To UBound(CharNames)
        ...
        ws.Cells(outRow, 4).Formula = "=ROUND(C" & outRow & "/1.875,1) / 86400"
...
Sub RunManual()
    ...
    For i = LBound(Lines) To UBound(Lines)
        If Trim(Lines(i)) <> "" Then
            ...
            ' Use IsEmptyOrWhitespace to check for all types of whitespace
            If ms.Cells(Row, 2).Value = "NARRATOR" And IsEmptyOrWhitespace(ms.Cells(Row, 3).Value) Then
                ' This row is empty - don't increment counter
            Else
                Row = Row + 1
            End If
       End If
    ...
    Set allocForm.ManualSheet = ms
    allocForm.Setup          ' <-- explicitly trigger data loading now that msSheet is set
...
Sub DiagnosticTest()
    ...
    ' Test 1: Check if UserForm exists in project
    Dim comp As Object
    Dim found As Boolean
    found = False
    ...
    For Each comp In ThisWorkbook.VBProject.VBComponents
        ...
        If comp.Name = "ManualCharNameAllocation" Then
            found = True
            Debug.Print "*** FOUND ManualCharNameAllocation ***"
        End If
    Next comp
    
    If Not found Then
        Debug.Print "ERROR: ManualCharNameAllocation NOT found in project!"
    End If
    
    ' Test 2: Try different declaration methods
    Debug.Print vbCrLf & "=== TESTING DECLARATIONS ==="
    
    ' Method A: Direct
    Dim testForm1 As ManualCharNameAllocation
    Debug.Print "Method A: Dim testForm1 As ManualCharNameAllocation - Compiles: " & (Err.Number = 0)
    Err.Clear
    
    ' Method B: Object with CreateObject (won't work, but let's see error)
    Dim testForm2 As Object
    On Error Resume Next
    Set testForm2 = CreateObject("ManualCharNameAllocation")
    If Err.Number <> 0 Then
        Debug.Print "Method B: CreateObject failed - " & Err.Description
    Else
        Debug.Print "Method B: CreateObject succeeded"
    End If
    Err.Clear
    
    ' Method C: Using UserForms collection
    On Error Resume Next
    Dim testForm3 As Object
    Set testForm3 = UserForms.Add("ManualCharNameAllocation")
    If Err.Number <> 0 Then
        Debug.Print "Method C: UserForms.Add failed - " & Err.Description
        Debug.Print "Error number: " & Err.Number
    Else
        Debug.Print "Method C: UserForms.Add succeeded"
        testForm3.Show
    End If
...
ErrorHandler:
    Debug.Print "ERROR in diagnostic: " & Err.Description & " (Number: " & Err.Number & ")"
End Sub
Sub TestFormOnly()
    Dim allocForm As ManualCharNameAllocation
    Set allocForm = New ManualCharNameAllocation
    Set allocForm.ManualSheet = ThisWorkbook.Sheets("Manual")
    allocForm.Setup
    allocForm.Show vbModeless
End Sub
